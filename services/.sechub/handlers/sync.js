...
async function sync(event) {
  ...
  if (repoProjects.length) await assignIssuesToRepositoryProjects(issues, repoProjects);
  if (orgProjects.length) await assignIssuesToOrganizationProjects(issues, orgProjects);
}
